// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from pure_pursuit:action/PathAndFeedback.idl
// generated code does not contain a copyright notice
#include "pure_pursuit/action/detail/path_and_feedback__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `path_with_angles`
#include "pure_pursuit/msg/detail/path2_d_with_angles__functions.h"
// Member `feedback_indices`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
pure_pursuit__action__PathAndFeedback_Goal__init(pure_pursuit__action__PathAndFeedback_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // path_with_angles
  if (!pure_pursuit__msg__Path2DWithAngles__init(&msg->path_with_angles)) {
    pure_pursuit__action__PathAndFeedback_Goal__fini(msg);
    return false;
  }
  // feedback_indices
  if (!rosidl_runtime_c__int32__Sequence__init(&msg->feedback_indices, 0)) {
    pure_pursuit__action__PathAndFeedback_Goal__fini(msg);
    return false;
  }
  return true;
}

void
pure_pursuit__action__PathAndFeedback_Goal__fini(pure_pursuit__action__PathAndFeedback_Goal * msg)
{
  if (!msg) {
    return;
  }
  // path_with_angles
  pure_pursuit__msg__Path2DWithAngles__fini(&msg->path_with_angles);
  // feedback_indices
  rosidl_runtime_c__int32__Sequence__fini(&msg->feedback_indices);
}

bool
pure_pursuit__action__PathAndFeedback_Goal__are_equal(const pure_pursuit__action__PathAndFeedback_Goal * lhs, const pure_pursuit__action__PathAndFeedback_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // path_with_angles
  if (!pure_pursuit__msg__Path2DWithAngles__are_equal(
      &(lhs->path_with_angles), &(rhs->path_with_angles)))
  {
    return false;
  }
  // feedback_indices
  if (!rosidl_runtime_c__int32__Sequence__are_equal(
      &(lhs->feedback_indices), &(rhs->feedback_indices)))
  {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_Goal__copy(
  const pure_pursuit__action__PathAndFeedback_Goal * input,
  pure_pursuit__action__PathAndFeedback_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // path_with_angles
  if (!pure_pursuit__msg__Path2DWithAngles__copy(
      &(input->path_with_angles), &(output->path_with_angles)))
  {
    return false;
  }
  // feedback_indices
  if (!rosidl_runtime_c__int32__Sequence__copy(
      &(input->feedback_indices), &(output->feedback_indices)))
  {
    return false;
  }
  return true;
}

pure_pursuit__action__PathAndFeedback_Goal *
pure_pursuit__action__PathAndFeedback_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Goal * msg = (pure_pursuit__action__PathAndFeedback_Goal *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_Goal));
  bool success = pure_pursuit__action__PathAndFeedback_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_Goal__destroy(pure_pursuit__action__PathAndFeedback_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_Goal__Sequence__init(pure_pursuit__action__PathAndFeedback_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Goal * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_Goal *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_Goal__Sequence__fini(pure_pursuit__action__PathAndFeedback_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_Goal__Sequence *
pure_pursuit__action__PathAndFeedback_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Goal__Sequence * array = (pure_pursuit__action__PathAndFeedback_Goal__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_Goal__Sequence__destroy(pure_pursuit__action__PathAndFeedback_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_Goal__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_Goal__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_Goal__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_Goal__Sequence * input,
  pure_pursuit__action__PathAndFeedback_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_Goal * data =
      (pure_pursuit__action__PathAndFeedback_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
pure_pursuit__action__PathAndFeedback_Result__init(pure_pursuit__action__PathAndFeedback_Result * msg)
{
  if (!msg) {
    return false;
  }
  // final_index
  return true;
}

void
pure_pursuit__action__PathAndFeedback_Result__fini(pure_pursuit__action__PathAndFeedback_Result * msg)
{
  if (!msg) {
    return;
  }
  // final_index
}

bool
pure_pursuit__action__PathAndFeedback_Result__are_equal(const pure_pursuit__action__PathAndFeedback_Result * lhs, const pure_pursuit__action__PathAndFeedback_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // final_index
  if (lhs->final_index != rhs->final_index) {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_Result__copy(
  const pure_pursuit__action__PathAndFeedback_Result * input,
  pure_pursuit__action__PathAndFeedback_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // final_index
  output->final_index = input->final_index;
  return true;
}

pure_pursuit__action__PathAndFeedback_Result *
pure_pursuit__action__PathAndFeedback_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Result * msg = (pure_pursuit__action__PathAndFeedback_Result *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_Result));
  bool success = pure_pursuit__action__PathAndFeedback_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_Result__destroy(pure_pursuit__action__PathAndFeedback_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_Result__Sequence__init(pure_pursuit__action__PathAndFeedback_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Result * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_Result *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_Result__Sequence__fini(pure_pursuit__action__PathAndFeedback_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_Result__Sequence *
pure_pursuit__action__PathAndFeedback_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Result__Sequence * array = (pure_pursuit__action__PathAndFeedback_Result__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_Result__Sequence__destroy(pure_pursuit__action__PathAndFeedback_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_Result__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_Result__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_Result__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_Result__Sequence * input,
  pure_pursuit__action__PathAndFeedback_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_Result * data =
      (pure_pursuit__action__PathAndFeedback_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
pure_pursuit__action__PathAndFeedback_Feedback__init(pure_pursuit__action__PathAndFeedback_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_index
  return true;
}

void
pure_pursuit__action__PathAndFeedback_Feedback__fini(pure_pursuit__action__PathAndFeedback_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_index
}

bool
pure_pursuit__action__PathAndFeedback_Feedback__are_equal(const pure_pursuit__action__PathAndFeedback_Feedback * lhs, const pure_pursuit__action__PathAndFeedback_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_index
  if (lhs->current_index != rhs->current_index) {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_Feedback__copy(
  const pure_pursuit__action__PathAndFeedback_Feedback * input,
  pure_pursuit__action__PathAndFeedback_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_index
  output->current_index = input->current_index;
  return true;
}

pure_pursuit__action__PathAndFeedback_Feedback *
pure_pursuit__action__PathAndFeedback_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Feedback * msg = (pure_pursuit__action__PathAndFeedback_Feedback *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_Feedback));
  bool success = pure_pursuit__action__PathAndFeedback_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_Feedback__destroy(pure_pursuit__action__PathAndFeedback_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_Feedback__Sequence__init(pure_pursuit__action__PathAndFeedback_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Feedback * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_Feedback *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_Feedback__Sequence__fini(pure_pursuit__action__PathAndFeedback_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_Feedback__Sequence *
pure_pursuit__action__PathAndFeedback_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_Feedback__Sequence * array = (pure_pursuit__action__PathAndFeedback_Feedback__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_Feedback__Sequence__destroy(pure_pursuit__action__PathAndFeedback_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_Feedback__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_Feedback__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_Feedback__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_Feedback__Sequence * input,
  pure_pursuit__action__PathAndFeedback_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_Feedback * data =
      (pure_pursuit__action__PathAndFeedback_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "pure_pursuit/action/detail/path_and_feedback__functions.h"

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Request__init(pure_pursuit__action__PathAndFeedback_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    pure_pursuit__action__PathAndFeedback_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!pure_pursuit__action__PathAndFeedback_Goal__init(&msg->goal)) {
    pure_pursuit__action__PathAndFeedback_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Request__fini(pure_pursuit__action__PathAndFeedback_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  pure_pursuit__action__PathAndFeedback_Goal__fini(&msg->goal);
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Request__are_equal(const pure_pursuit__action__PathAndFeedback_SendGoal_Request * lhs, const pure_pursuit__action__PathAndFeedback_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!pure_pursuit__action__PathAndFeedback_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Request__copy(
  const pure_pursuit__action__PathAndFeedback_SendGoal_Request * input,
  pure_pursuit__action__PathAndFeedback_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!pure_pursuit__action__PathAndFeedback_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

pure_pursuit__action__PathAndFeedback_SendGoal_Request *
pure_pursuit__action__PathAndFeedback_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_SendGoal_Request * msg = (pure_pursuit__action__PathAndFeedback_SendGoal_Request *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Request));
  bool success = pure_pursuit__action__PathAndFeedback_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Request__destroy(pure_pursuit__action__PathAndFeedback_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__init(pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_SendGoal_Request * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_SendGoal_Request *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__fini(pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence *
pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * array = (pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__destroy(pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * input,
  pure_pursuit__action__PathAndFeedback_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_SendGoal_Request * data =
      (pure_pursuit__action__PathAndFeedback_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Response__init(pure_pursuit__action__PathAndFeedback_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    pure_pursuit__action__PathAndFeedback_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Response__fini(pure_pursuit__action__PathAndFeedback_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Response__are_equal(const pure_pursuit__action__PathAndFeedback_SendGoal_Response * lhs, const pure_pursuit__action__PathAndFeedback_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Response__copy(
  const pure_pursuit__action__PathAndFeedback_SendGoal_Response * input,
  pure_pursuit__action__PathAndFeedback_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

pure_pursuit__action__PathAndFeedback_SendGoal_Response *
pure_pursuit__action__PathAndFeedback_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_SendGoal_Response * msg = (pure_pursuit__action__PathAndFeedback_SendGoal_Response *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Response));
  bool success = pure_pursuit__action__PathAndFeedback_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Response__destroy(pure_pursuit__action__PathAndFeedback_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__init(pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_SendGoal_Response * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_SendGoal_Response *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__fini(pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence *
pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * array = (pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__destroy(pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * input,
  pure_pursuit__action__PathAndFeedback_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_SendGoal_Response * data =
      (pure_pursuit__action__PathAndFeedback_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
pure_pursuit__action__PathAndFeedback_GetResult_Request__init(pure_pursuit__action__PathAndFeedback_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    pure_pursuit__action__PathAndFeedback_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Request__fini(pure_pursuit__action__PathAndFeedback_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Request__are_equal(const pure_pursuit__action__PathAndFeedback_GetResult_Request * lhs, const pure_pursuit__action__PathAndFeedback_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Request__copy(
  const pure_pursuit__action__PathAndFeedback_GetResult_Request * input,
  pure_pursuit__action__PathAndFeedback_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

pure_pursuit__action__PathAndFeedback_GetResult_Request *
pure_pursuit__action__PathAndFeedback_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_GetResult_Request * msg = (pure_pursuit__action__PathAndFeedback_GetResult_Request *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Request));
  bool success = pure_pursuit__action__PathAndFeedback_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Request__destroy(pure_pursuit__action__PathAndFeedback_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__init(pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_GetResult_Request * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_GetResult_Request *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__fini(pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence *
pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * array = (pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__destroy(pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * input,
  pure_pursuit__action__PathAndFeedback_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_GetResult_Request * data =
      (pure_pursuit__action__PathAndFeedback_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "pure_pursuit/action/detail/path_and_feedback__functions.h"

bool
pure_pursuit__action__PathAndFeedback_GetResult_Response__init(pure_pursuit__action__PathAndFeedback_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!pure_pursuit__action__PathAndFeedback_Result__init(&msg->result)) {
    pure_pursuit__action__PathAndFeedback_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Response__fini(pure_pursuit__action__PathAndFeedback_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  pure_pursuit__action__PathAndFeedback_Result__fini(&msg->result);
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Response__are_equal(const pure_pursuit__action__PathAndFeedback_GetResult_Response * lhs, const pure_pursuit__action__PathAndFeedback_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!pure_pursuit__action__PathAndFeedback_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Response__copy(
  const pure_pursuit__action__PathAndFeedback_GetResult_Response * input,
  pure_pursuit__action__PathAndFeedback_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!pure_pursuit__action__PathAndFeedback_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

pure_pursuit__action__PathAndFeedback_GetResult_Response *
pure_pursuit__action__PathAndFeedback_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_GetResult_Response * msg = (pure_pursuit__action__PathAndFeedback_GetResult_Response *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Response));
  bool success = pure_pursuit__action__PathAndFeedback_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Response__destroy(pure_pursuit__action__PathAndFeedback_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__init(pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_GetResult_Response * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_GetResult_Response *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__fini(pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence *
pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * array = (pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__destroy(pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * input,
  pure_pursuit__action__PathAndFeedback_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_GetResult_Response * data =
      (pure_pursuit__action__PathAndFeedback_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "pure_pursuit/action/detail/path_and_feedback__functions.h"

bool
pure_pursuit__action__PathAndFeedback_FeedbackMessage__init(pure_pursuit__action__PathAndFeedback_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    pure_pursuit__action__PathAndFeedback_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!pure_pursuit__action__PathAndFeedback_Feedback__init(&msg->feedback)) {
    pure_pursuit__action__PathAndFeedback_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
pure_pursuit__action__PathAndFeedback_FeedbackMessage__fini(pure_pursuit__action__PathAndFeedback_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  pure_pursuit__action__PathAndFeedback_Feedback__fini(&msg->feedback);
}

bool
pure_pursuit__action__PathAndFeedback_FeedbackMessage__are_equal(const pure_pursuit__action__PathAndFeedback_FeedbackMessage * lhs, const pure_pursuit__action__PathAndFeedback_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!pure_pursuit__action__PathAndFeedback_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_FeedbackMessage__copy(
  const pure_pursuit__action__PathAndFeedback_FeedbackMessage * input,
  pure_pursuit__action__PathAndFeedback_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!pure_pursuit__action__PathAndFeedback_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

pure_pursuit__action__PathAndFeedback_FeedbackMessage *
pure_pursuit__action__PathAndFeedback_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_FeedbackMessage * msg = (pure_pursuit__action__PathAndFeedback_FeedbackMessage *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pure_pursuit__action__PathAndFeedback_FeedbackMessage));
  bool success = pure_pursuit__action__PathAndFeedback_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pure_pursuit__action__PathAndFeedback_FeedbackMessage__destroy(pure_pursuit__action__PathAndFeedback_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pure_pursuit__action__PathAndFeedback_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__init(pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_FeedbackMessage * data = NULL;

  if (size) {
    data = (pure_pursuit__action__PathAndFeedback_FeedbackMessage *)allocator.zero_allocate(size, sizeof(pure_pursuit__action__PathAndFeedback_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pure_pursuit__action__PathAndFeedback_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pure_pursuit__action__PathAndFeedback_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__fini(pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pure_pursuit__action__PathAndFeedback_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence *
pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * array = (pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence *)allocator.allocate(sizeof(pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__destroy(pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__are_equal(const pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * lhs, const pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence__copy(
  const pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * input,
  pure_pursuit__action__PathAndFeedback_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pure_pursuit__action__PathAndFeedback_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pure_pursuit__action__PathAndFeedback_FeedbackMessage * data =
      (pure_pursuit__action__PathAndFeedback_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pure_pursuit__action__PathAndFeedback_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pure_pursuit__action__PathAndFeedback_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pure_pursuit__action__PathAndFeedback_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
